...
```text
05myapp-container-kafka-1             Up 12 seconds             0.0.0.0:19092->9092/tcp
05myapp-container-kafka-connect-1     Up 12 seconds             0.0.0.0:18093->8083/tcp
05myapp-container-kafka-ui-1          Up 12 seconds             0.0.0.0:18085->8080/tcp
05myapp-container-mariadb-1           Up 12 seconds (healthy)   0.0.0.0:13306->3306/tcp
05myapp-container-order-service-1     Up 6 seconds              0.0.0.0:18083->8080/tcp
```
...
```bash
docker exec 05myapp-container-mariadb-1 mariadb -ucloud -p'Skala25a!23$' order_service_db -e "select id, aggregate_type, aggregate_id, event_type, created_at from outbox_event order by created_at desc;"
docker exec 05myapp-container-mariadb-1 mariadb -ucloud -p'Skala25a!23$' order_service_db -e "select aggregate_id, event_type, left(payload, 300) as payload_preview from outbox_event order by created_at desc;"
```
...
docker exec 05myapp-container-mariadb-1 mariadb -uroot -prootpass -e "show variables like 'binlog_row_image';"
docker exec 05myapp-container-mariadb-1 mariadb -uroot -prootpass -e "show grants for 'cloud'@'%';"
```
...
docker exec 05myapp-container-kafka-1 kafka-console-consumer \
